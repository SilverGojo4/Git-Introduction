...
# Git Push 用於將本地 Repository 的提交推送到<br>遠端 Repository，便團隊協作和版本控制
### 1. 創建新的遠端分支並推送
```zsh
...
## 3. 設置上游分支，以便將來的推送可以簡化（只需第一次推送時設置）
```zsh
...
# Git Pull 用於將遠端 Repository 最新更改合併到當前分支
## 1. 更新當前本地分支
```zsh
...
# Git Clone 用於從遠端 Repository 克隆一份副本到本地
## 1. 克隆遠端 Repository
```zsh
# 克隆遠端儲存庫到本地目錄
$ git clone [遠端儲存庫URL]
